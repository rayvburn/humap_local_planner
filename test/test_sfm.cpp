...
TEST(SocialForceModel, computeInternalForce) {
    Vector robot_pose1(0.0, 0.0, 0.0);
    Vector robot_vel1(0.0, 0.0, 0.0);
    Vector target1(5.0, 0.0, 0.0);
    Vector robot_target_dist1(target1 - robot_pose1);
    Vector force1 = SfmExposed::computeInternalForce(robot_vel1, robot_target_dist1, 100.0, 1.0, 0.5);
    Angle force1_dir(force1);
    ASSERT_DOUBLE_EQ(0.0, force1_dir.getRadian());


    Vector robot_pose2(-5.0, -10.0, -3.0 * IGN_PI_4);
    Vector robot_vel2(-1.0, -1.0, 0.0);
    Vector target2(-10.0, -15.0, 0.0);
    Vector robot_target_dist2(target2 - robot_pose2);
    Vector force2 = SfmExposed::computeInternalForce(robot_vel2, robot_target_dist2, 1.0, 10.0, 2.0);
    Angle force2_dir(force2);
    // keep the current velocity direction
    ASSERT_DOUBLE_EQ(-3.0 * IGN_PI_4, force2_dir.getRadian());


    Vector robot_pose3(0.0, 0.0, IGN_PI_2);
    Vector robot_vel3(+0.5, -1.0, 0.0);
    Vector target3(+5.0, -10.0, 0.0);
    Vector robot_target_dist3(target3 - robot_pose3);
    Vector force3 = SfmExposed::computeInternalForce(robot_vel3, robot_target_dist3, 1.0, 10.0, 2.0);
    Angle force3_dir(force3);
    ASSERT_NEAR(-1.1071, force3_dir.getRadian(), 1e-04); // atan2(-10, 5)


    // zero mass
    Vector force3_no_mass = SfmExposed::computeInternalForce(robot_vel3, robot_target_dist3, 0.0, 10.0, 2.0);
...
TEST(SocialForceModel, computeThetaAlphaBetaAngle2011) {
    ...
    Vector robot_vel1(0.0, 0.0, 0.0);
    Vector object_vel1(1.0, -1.0, 0.0);
    ...
    Vector robot_vel2(2.0, 3.0, 0.3);
    Vector object_vel2(1.0, -1.0, 0.0);
    Angle theta2 = SfmExposed::computeThetaAlphaBetaAngle2011(robot_vel2, object_vel2);
    EXPECT_NEAR(theta2.getRadian(), 1.7675, 1e-04);

    Vector robot_vel3(1.0, -1.0, 0.0);
    Vector object_vel3(2.0, 3.0, 0.3);
...
TEST(SocialForceModel, computeThetaAlphaBetaAngle2014) {
    // angle difference
    Vector n_alpha1(1.0, 0.0, 0.0);
    Vector d_alpha_beta1(2.0, 3.0, 1.0);
    ...
    Vector n_alpha2(Angle(-IGN_PI_4));
    Vector d_alpha_beta2(Angle(+IGN_PI_2));
...
TEST(SocialForceModel, computeThetaAlphaBetaAngle) {
    ...
    Angle robot_yaw1(-IGN_PI_4);
    Angle object_yaw1(+IGN_PI_2);
    ...
    Angle robot_yaw2(IGN_PI_4);
    Angle object_yaw2(IGN_PI_2);
...
TEST(SocialForceModel, computeNormalAlphaDirection) {
    ...
    Angle robot_yaw1(IGN_PI_4);
    Vector robot_n1 = SfmExposed::computeNormalAlphaDirection(robot_yaw1, sfm::PARAMETER_DESCRIPTION_2011);
    Angle robot_n1_dir = robot_n1.calculateDirection();
    Angle expected1(robot_yaw1.getRadian() + IGN_PI);
    EXPECT_DOUBLE_EQ(expected1.getRadian(), robot_n1_dir.getRadian());

    Angle robot_yaw2(3.0 * IGN_PI_4);
    Vector robot_n2 = SfmExposed::computeNormalAlphaDirection(robot_yaw2, sfm::PARAMETER_DESCRIPTION_2011);
    Angle robot_n2_dir = robot_n2.calculateDirection();
    Angle expected2(robot_yaw2.getRadian() + IGN_PI);
    EXPECT_DOUBLE_EQ(expected2.getRadian(), robot_n2_dir.getRadian());

    // points in the direction of the robot motion
    Angle robot_yaw3(3.0 * IGN_PI_4);
    Vector robot_n3 = SfmExposed::computeNormalAlphaDirection(robot_yaw3, sfm::PARAMETER_DESCRIPTION_2014);
    Angle robot_n3_dir = robot_n3.calculateDirection();
    Angle expected3(robot_yaw3.getRadian());
    EXPECT_DOUBLE_EQ(expected3.getRadian(), robot_n3_dir.getRadian());

    Angle robot_yaw4(-IGN_PI_2);
    Vector robot_n4 = SfmExposed::computeNormalAlphaDirection(robot_yaw4, sfm::PARAMETER_DESCRIPTION_2014);
    Angle robot_n4_dir = robot_n4.calculateDirection();
    Angle expected4(robot_yaw4.getRadian());
...
TEST(SocialForceModel, computePerpendicularToNormal) {
    ...
    Vector n_a1a_expected = n_a1.rotated(IGN_PI_2);
    ...
    Vector p_a1b = SfmExposed::computePerpendicularToNormal(n_a1, rel_loc1, param_desc_1b);
    Vector n_a1b_expected = n_a1.rotated(-IGN_PI_2);
    ...
    Vector n_a2a_expected = n_a2.rotated(-IGN_PI_2);
    ...
    Vector p_a2b = SfmExposed::computePerpendicularToNormal(n_a2, rel_loc2, param_desc_2b);
    Vector n_a2b_expected = n_a2.rotated(+IGN_PI_2);
