...
//	void init(std::shared_ptr<const ros_interface::ParamLoader> params_ptr,
...
//			const ignition::math::Pose3d &actor_pose, const Vector3 &actor_velocity,
//			const Vector3 &actor_target, const core::CommonInfo &actor_info,
//			const double &dt, const std::vector<std::string> &ignored_models_v);
	...
	/// social force
	ignition::math::Pose3d computeNewPose(const ignition::math::Pose3d &actor_pose, const Vector3 &actor_vel,
			const Vector3 &social_force, const Vector3 &target, const double &dt);

	/// \brief Resets class' internal state - mainly forces but resets internal classes too
	void reset();

	/// \brief Returns internal force vector
	Vector3 getForceInternal() const;
	/// \brief Returns interaction force vector
	Vector3 getForceInteraction() const;
	/// \brief Returns combined force vector (internal
	/// and interaction components summed up)
	Vector3 getForceCombined() const;
...
//			const bool &is_actor);
	...
	/// called dictionary here. Uses core::Target static function as helper.
	...
	bool isDynamicObstacle(const Vector3 &vel) const;
	...
	/// \param v is a vector
	/// \return Angle
	double computeVectorDirection(const Vector3 &v) const;
	...
	/// Vector3 model_vel, inflation::Border* model_border_ptr
//	std::tuple<bool, bool, std::string, ignition::math::Pose3d, Vector3, inflation::Border* /* std::shared_ptr<inflation::Border> */ >
//	preprocessTypicalModel(const gazebo::physics::WorldPtr &world_ptr, const size_t &model_num,
//			const std::vector<std::string> &ignored_models_v, const core::CommonInfo &actor_info,
//			inflation::Box &model_box);
	...
	/// Vector3 model_vel, inflation::Border* model_border_ptr
//	std::tuple<bool, bool, std::string, ignition::math::Pose3d, Vector3, inflation::Border* /* std::shared_ptr<inflation::Border> */ >
//	preprocessWorldBoundary(const size_t &wall_num, inflation::Box &model_box);
