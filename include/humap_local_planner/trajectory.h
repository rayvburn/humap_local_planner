 ...
 * `humap_local_planner` in CMakeLists) can't be unit-tested anymore:
 * ```
...
public:
	...
	double getTime(const size_t& idx) const {
		if (!requestedValidIndex(idx)) {
			throw std::runtime_error("Wrong index requested in humap_local_planner::Trajectory::getTime");
		}
		return dt_ * idx;
	}
	...
	geometry::Pose getPose(const size_t& idx) const {
		if (!requestedValidIndex(idx)) {
			throw std::runtime_error("Wrong index requested in humap_local_planner::Trajectory::getPose");
		}
	...
	geometry::Vector getVelocity(const size_t& idx) const {
		if (!requestedValidIndex(idx)) {
			throw std::runtime_error("Wrong index requested in humap_local_planner::Trajectory::getVelocity");
		}
		return vels_.at(idx);
	}
	...
	std::vector<std::pair<geometry::Pose, geometry::Vector>> getStates() const {
		if (poses_.size() != vels_.size()) {
			throw std::runtime_error("Wrong index requested in humap_local_planner::Trajectory::getState");
		}
