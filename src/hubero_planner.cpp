...
HuberoPlanner::~HuberoPlanner() {
	...
	printf(
		"HuberoPlanner destructor: %5.2f%% of selected trajectories were generated by the SocialTrajectoryGenerator\r\n",
		stg_percentage
	);
}
...
bool HuberoPlanner::computeCellCost(int cx, int cy, std::map<std::string, float>& costs) {
...
// static
double HuberoPlanner::computeDistanceLimits(
...
Pose HuberoPlanner::getPoseFromPlan(
...
bool HuberoPlanner::planYieldWayCrossing() {
...
bool HuberoPlanner::planRecoveryRotateAndRecede() {
...
bool HuberoPlanner::collectTrajectoryMotionData(const base_local_planner::Trajectory& best_traj) {
	motion_data_.behaviour_active_ = "unknown";
	motion_data_.force_internal_ = geometry::Vector();
	motion_data_.force_interaction_ = geometry::Vector();
	motion_data_.force_social_ = geometry::Vector();
