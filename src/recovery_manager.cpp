...
RecoveryManager::RecoveryManager():
	...
	recovery_goal_x_(NAN),
	recovery_goal_y_(NAN),
	recovery_goal_th_(NAN),
	prev_cmd_vx(0.0),
	prev_cmd_omega(0.0)
{}
...
bool RecoveryManager::planRecoveryRotateAndRecede(
	...
	bool recovery_goal_valid =
		!std::isinf(recovery_goal_x_)
		&& !std::isinf(recovery_goal_y_)
		&& !std::isinf(recovery_goal_th_);
	if (recovery_goal_valid) {
		auto rgoal_cost = obstacle_costfun.getFootprintCost(recovery_goal_x_, recovery_goal_y_, recovery_goal_th_);
	...
	for (int i = 0; i < RECOVERY_ROTATE_AND_RECEDE_SURROUNDING_PTS; i++) {
		...
		recovery_goal_x_ = poses_eval.back().getX();
		recovery_goal_y_ = poses_eval.back().getY();
		recovery_goal_th_ = poses_eval.back().getZ();
		return true;
	}
...
void RecoveryManager::resetRecoveryGoal() {
	recovery_goal_x_ = NAN;
	recovery_goal_y_ = NAN;
	recovery_goal_th_ = NAN;
}
