...
% helps to detect reaching the goal pose (to finish the simulation)
prev_dist_to_goal = Inf;

figure;
for i=1:1e06
    dist_to_goal = norm(GOAL_POSE(1:2) - robot_pose(1:2));
    if dist_to_goal <= 0.0001 || prev_dist_to_goal < dist_to_goal
        fprintf("Successfully finished the simulation\n")
        break;
    end
    ...
    %plot(i * SIM_PERIOD, vel_local(1), 'ro')
    hold on;
    plot(dist_to_goal, vel_local(2), 'go')
    %plot(i * SIM_PERIOD, vel_local(2), 'go')
